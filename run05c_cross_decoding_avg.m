...
% tt_on_test{1}         = target_times + smoothing_window_mu * -0.5; % target times onset
% tt_on_test{2}         = target_times + smoothing_window_beta * -0.5; % target times onset
% tt_on_model{1}        = repmat(1.66 + smoothing_window_mu * -0.5, numel(tt_on_test{1}), 1); % 1.66 is max accuracy mu
% tt_on_model{2}        = repmat(2.23 + smoothing_window_beta * -0.5, numel(tt_on_test{2}), 1); % 2.23 is center of peak with max accuracy in beta
target_frequencies    = {[11 14]; ...
                         [20 30]};
%conditions = {'mouth' ,'forward'};
...
for tp1 = 1:size(tpoints_model, 1)
for tp2 = 1:size(tpoints, 1)
    [~, res] = mv_classify(cfg, Xz(:, :, tpoints_model(tp1, 1):tpoints_model(tp1, 2)), clabel_1, ...
                                Xz2(:, :, tpoints(tp2, 1):tpoints(tp2, 2)), clabel_2); % returns probability for clabel == 1
    res_mat = [repmat(times_out(tp2), numel(res.testlabel), 1), res.testlabel, res.perf{1}, abs(res.perf{1} - (res.testlabel - 1)), test_set.trialinfo.rating]
    perf(tp2,c2_number) = res_mat;
    
end % end time points
end
%permformance_per_condition(:, :, c2_number) = perf;
end % end conditions2
%end % end conditions1
if t_freqs == 1
    fnameout = [resdir, subjects{file_nr}, '_cross_classification_mu_prob.mat']; 
else
    fnameout = [resdir, subjects{file_nr}, '_cross_classification_beta_prob.mat'];
end
