...
%% CSP
smoothing_window{1}  = 0.56; % in s; corresponds to 7 cycles of center freqency, 1/12.5*7
smoothing_window{2}  = 0.28; % in s; corresponds to 7 cycles of center freqency, 1/25*7
...
for tp = 1:numel(target_times)
    on_off = target_times(tp) + (smoothing_window{t_freqs} * [-0.5, 0.5]);
...
save([resdir, 'decoding_real_7cycles.mat'], 'decoding_real');
end
%exit
