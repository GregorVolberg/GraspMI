function [] = run06b_cross_classification_auc_plot()
%% paths and files
bidsPath = '../bids/';
...
mu_files   = strcat(resdir, subjects, '_cross_classification_mu_auc.mat');
beta_files = strcat(resdir, subjects, '_cross_classification_beta_auc.mat');
...
for freq_band = 1:numel(res_files)
    ...
    for ddim1 = 1:size(data, 4)
        for ddim2 = 1:size(data, 5)
            ...
            [h,p,~,t]=ttest(data(:, :,:, ddim1, ddim2), 0.5, 'Tail', 'right', 'Alpha', alpha_1stlevel/2);
            bw_matrix = squeeze(bwlabeln(h));
            %bw_matrix = bwlabeln(orig);
            bw_bins   = histcounts(bw_matrix, (1:(max(bw_matrix(:))+1))-0.5); % do not count zeros
            n_cluster = unique(bw_matrix(bw_matrix>0));
            bw_tclustersum_cell{freq_band, ddim1, ddim2} = arrayfun(@(x) sum(t.tstat(bw_matrix == x)), n_cluster);
            p_cell{freq_band, ddim1, ddim2} = p;
...
for freq_band = 1:numel(res_files)
    ...
    [h,~,~,t]=ttest(rand_data, 0.5, 'Tail', 'right', 'Alpha', alpha_1stlevel/2);
    rand_nums = bwlabeln(h);
    n_cluster = unique(rand_nums(rand_nums>0));
    tclustersum_cell{freq_band, rnd} = arrayfun(@(x) sum(t.tstat(rand_nums == x)), n_cluster);
    %p_cell{freq_band, ddim1, ddim2} = p;
...
for ffreq = 1:size(bw_bins_cell,1)
    ...
    tdat = squeeze(bw_tclustersum_cell(ffreq, :, :));
    perm_tclust = cell2mat([tclustersum_cell(ffreq,:)]');
for ddim1 = 1:3
    for ddim2 = 1:3
        erg{ffreq, ddim1, ddim2}  = find(fdat{ddim1,ddim2} > n_crit(ffreq));
        ergt{ffreq, ddim1, ddim2} = find(tdat{ddim1,ddim2} > quantile(perm_tclust, 1-perm_alpha/2));
        cluster_map{ffreq, ddim1, ddim2} = ismember(bw_matrix_cell{ffreq, ddim1, ddim2}, ergt{ffreq, ddim1, ddim2});
    end
end
end
...
for freq_band = 1:numel(res_files)
    ...
    for participant = 1:numel(data_cells)
        ...
        for ddim1 = 1:size(mdata, 3)
            ...
            p_alpha = flipud(squeeze(p_cell{freq_band, ddim1, ddim2}));
            p_alpha(p_alpha >= alpha_1stlevel/2) = 0.6;
            %p_alpha = p_alpha .* flipud(cluster_map{freq_band, ddim1, ddim2});
            alpha(gca, 1- p_alpha);
            ...
            % end
            if ddim1 == 3
            xlabel({'Real Mov. Time (s)'; [char(conditions{ddim2, 1}), ' vs. ', char(conditions{ddim2, 2})]; });    
            end
            if ddim2 == 1
            ylabel({[char(conditions{ddim1, 1}), ' vs. ', char(conditions{ddim1, 2})]; 'Imag. Mov. Time (s)'});
%            ylabel('test\Imag. Movement Time (s)');
            end
            if ddim1 == 2 && ddim2 == 1
             annotation(gcf,'textarrow', [0 0], [0.5 1], ...
            'String', ['Imagined movements (', tdata_name{freq_band}, ' AUC)'], ...
            'HeadStyle', 'none', 'LineStyle', 'none',...
            'FontSize',12, 'color','k', 'FontWeight','bold', 'TextRotation',90);
            end
            if ddim1 == 1 && ddim2 == 2
             annotation(gcf,'textarrow', [0.66 1], [0.98 1], ...
            'String', ['Real movements (', tdata_name{freq_band}, ' AUC)'], ...
            'HeadStyle', 'none', 'LineStyle', 'none',...
...
fig = gcf;
print(fig, [resdir, './Cross_classification_accuracy_auc_clustert_', tdata_name{freq_band}, '.svg'],'-dsvg');
print(fig, ['./md_images/Cross_classification_accuracy_auc_clustert_', tdata_name{freq_band}, '.png'],'-dpng');
close(gcf);
end % freq_band
