% =======================
% function[] = graspMI_practice()
% Two practice runs with 12 trials each and no saving of results
% ========================
function[] = graspMI_practice()
...
%% Stimulus
FixCrossSize  = 1; % in degrees visual angle
...
FixCrossWidth = 4; % line width, in pixels             
...
% BreakBetweenBlocks = 120; % pause between blocks (s)
...
try
    ...
    % prepare runs
    numBlocks                 = 2;  % must be even-numbered due to the blocking movement/imagery
    RepsPerBlock              = 4; % number of repetitions per block (mouth, shoulder, forward)
    ...
    PTBCueTime  = CueTime - (frame_s * 0.5); % 
    PTBTaskTime = TaskTime - (frame_s * 0.5);  
...
%% loop over blocks
     ...
     for nblock = 1:length(Runs)
      ...
      %% loop over trials                   
        ...
        if (isnan(keyCode) | ~ismember([89, 88, 67, 86], keyCode))
        rating = NaN;
        else
        rating = find(ismember([89, 88, 67, 86], keyCode));
        end
