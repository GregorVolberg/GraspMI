...
%% modified by Gregor
% Update colum 'onsets'
onsets_samples   = events.onset / (1/hdr.Fs); % onset in samples
movement_samples = round(events.mov_onset / (1/hdr.Fs)) + (mod(events.mov_onset, 1/hdr.Fs) >= 1/hdr.Fs/2); % rounded to nearest sample
movement_samples(isnan(movement_samples)) = 0;
events.onset  = (onsets_samples + movement_samples) * (1/hdr.Fs);

% check whether movement onsets are within reasonable range
% mark all invalid with RTs > 2 s where time to next trial would be < 3 s
%mark_invalid = find((movement_samples * (1/hdr.Fs)) > cfg.max_RT);
%events.valid_movement(mark_invalid) = {'no'};
