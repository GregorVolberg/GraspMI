...
rawdatafile = [participant, 'motion.txt'];
protocolFileName = [rawPath, 'sub-', participant, '_task-Grasping.mat'];

%% polhemus csv to ft raw data
SRATE    = 240; % sampling rate is 240 Hz, 
tmp = readtable([rawPath, rawdatafile]);
tmp = tmp(:, 2:7);
tmp = renamevars(tmp, "X_IN_____", "X");
tmp = renamevars(tmp, "Y_IN_____", "Y");
tmp = renamevars(tmp, "Z_IN_____", "Z");

motionData = unstack(tmp, ["X", "Y", "Z"], "Sns", 'VariableNamingRule', 'preserve'); clear tmp
time_axis  = [0:size(motionData,1)-1]' * (1/SRATE); %the time axis, , relative to start of recodring, is not saved anywere
button1    = motionData.BB; % Button 1 of sensor 5 
mdata      = motionData(:, {'X_1', 'Y_1', 'Z_1', ...
                       'X_2', 'Y_2', 'Z_2', ...
                       'X_3', 'Y_3', 'Z_3'}); % the motion data
...
button1(1:20000) = 0; % seen from plot
find((diff(movmean(button1, 4)) == 0.25) + 1 + 1;
IdxON  = find(diff(movmean(button1,4)) > 0.6)  + 1 - vpixxOffset_samples;
