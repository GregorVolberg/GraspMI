function [] = run05_cross_decoding_auc()
%% decoding
...
% subset EEG II
cfg            = [];
cfg.trials     = find(trials_imag & (trials_con1_2 | trials_con2_2));%find(trials_table.real_forward + (trials_table.real_mouth * 2));
cfg.keeptrials = 'yes';
test_set       = ft_timelockanalysis(cfg, eeg); 
clabel_2       = ismember(test_set.trialinfo.movement, conditions{c2_number, 1})+1;
...
cfg = [];
cfg.classifier          = 'lda';
cfg.metric              = 'auc'; % as recommended in MVLight Toolbox example scripts
cfg.k                   = 5;
cfg.repeat              = 100;
cfg.preprocess          = 'csp';
...
if t_freqs == 1
    fnameout = [resdir, subjects{file_nr}, '_cross_classification_mu_auc.mat']; 
else
    fnameout = [resdir, subjects{file_nr}, '_cross_classification_beta_auc.mat'];
end
